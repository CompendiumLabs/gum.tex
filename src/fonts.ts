...
// gum has already fetched the bytes for metrics, so hand those same bytes to
// the browser via the FontFace API rather than writing @font-face rules with
// urls — no second request per font. Faces arrive incrementally (the extra
// ones only when a formula needs them), so call this after every render and
// it registers whatever is new.
...
export function installFontFaces(): void {
  for (const name of MATH_FONTS) {
    ...
    if (installed.has(name) || !(data instanceof ArrayBuffer)) continue
